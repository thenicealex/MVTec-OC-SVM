...
class BaseSVDD(BaseEstimator, OutlierMixin):
    ...
    def fit(self, X, y=None, weight=None):
        ...
        # Set sample weights
        self.weight = np.ones((self.n_samples, 1), dtype=np.int64) if weight is None else weight
            
        # Validate and set gamma
        self._validate_gamma(X)
    ...
    def _validate_gamma(self, X):
        """Validate and set the gamma parameter."""
        if self.gamma == 0:
            raise ValueError("The gamma value of 0.0 is invalid. Use 'auto' to set gamma to 1 / n_features.")
        ...
        if isinstance(self.gamma, str):
            if self.gamma == "scale":
                ...
                raise ValueError("When 'gamma' is a string, it should be either 'scale' or 'auto'.")   
    ...
    def _get_kernel(self, X, Y=None):
        ...
        params = {"gamma": self.gamma, "degree": self.degree, "coef0": self.coef0} if not callable(self.kernel) else self.kernel_params or {}
